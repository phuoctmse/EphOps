...
- **LLM Provider:** AWS Bedrock (Amazon Nova) — serverless, pay-per-use.
...
- **LLM Cloud:** AWS SDK for JavaScript v3 (`@aws-sdk/client-bedrock-runtime`).
...
Phần này quy định cách Backend giao tiếp với LLM (AWS Bedrock / Amazon Nova là primary; Ollama là fallback cho local development) và các ràng buộc để đảm bảo AI hoạt động như một chuyên gia FinOps thực thụ.

### 5.1. System Prompt Strategy
AI Agent phải được khởi tạo với một System Prompt nghiêm ngặt. Prompt này được gửi qua **AWS Bedrock (Amazon Nova)** trong môi trường production; **Ollama** được giữ lại làm fallback khi Bedrock không khả dụng:
...
- **Mocking:** - Mock hoàn toàn các gọi API đến LLM (Bedrock / Ollama).
